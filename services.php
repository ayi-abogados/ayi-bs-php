...
$stmt = $conn->prepare("SELECT content FROM pages WHERE slug = 'services'");
$stmt->execute();
...
// Obtener el template
$htmlTemplate = file_get_contents('services.html');
...
$modalHtml = <<<HTML
    ...
    function loadAdminContent(type) {
        if (type === 'login') {
            ...
            document.getElementById('dashboardFrame').src = 'admin/dashboard.php?page=services';
        }
    }
...
// Reemplazar el contenido dinámico en el template
if (preg_match('/<main[^>]*>(.*?)<\/main>/s', $dynamicContent, $matches)) {
